...
int checkPupil(int i,int j)
{
	int flag=0;
	if(srcInv.at<uchar>(i+PUPILDIST,j+PUPILDIST)==255)
		flag++;
	if(srcInv.at<uchar>(i-PUPILDIST,j+PUPILDIST)==255)
		flag++;
	if(srcInv.at<uchar>(i+PUPILDIST,j-PUPILDIST)==255)
...
int main(int argc,char* argv[])
{
	...
	cout<<"usage: imageLocation PUPILDISTFLAG ";	
	
	src=imread(argv[1],0);
	PUPILDISTFLAG = atoi(argv[2]);
	...
	//eroding and dilating inverted image
	/*
	if(erodeflag)
	{
		erode(srcInv,srcInv);
	...
	for(int j=0;j<src.rows;j++)
		for(int i=0;i<src.cols;i++)
		{
			if(srcInv.at<uchar>(j,i))
			{
				...
				if(dot>maxValue&&checkPupil(i,j))
				{
					maxValue=dot;
					maxPoint.x=i;
					maxPoint.y=j;
