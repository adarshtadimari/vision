...
Mat fn_image(Mat img,string fname)
{
    Mat projected;
    float c1,c2,c3;
    //cout<<img.cols;
    Mat reshaped=img.reshape(1,img.rows*img.cols);
    //Mat finalMatrix = Mat::zeros(1216,120*120*3);
    string subname;     
    //cout<<reshaped;
    ...
    string cname;
    stringstream s; 
    int pos=fname.find(".jpg");
    subname=fname.substr(0,pos);
    int tr1,tr2;
    int count=0;
        ...
        for(tr2=0;tr2<=8;tr2+=4)
        {
            ...
            for(vpower=0.5;vpower<=1.5;vpower+=0.5)
            {
              ...
              // waitKey(0);
                ...
                finalMatrix.row(count)=rt2.row(0);   
