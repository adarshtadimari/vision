...
int distanceApply(int vecx , int vecy)
{
	if(vecx*vecx+vecy*vecy>DISTANCETHRESHOLD*DISTANCETHRESHOLD)
		return 0;
	else
		return 0;
}
	...
	*/	
	...
	for(int j=0;j<src.rows;j++)
		for(int i=0;i<src.cols/2;i++)
		{
			...
			{
				dot=0;
				int t1 = ((j-LOWDTHRESHOLD)>0)?(j-LOWDTHRESHOLD):0;
				int t2 = ((j+LOWDTHRESHOLD)>src.rows)?src.rows:(j+LOWDTHRESHOLD);
				for(int j2= t1;j2<t2;j2++)
				{
					for(int i2=0;i2<src.cols/2;i2++)
					{
					...
					//printf("%f at(%d,%d)",p,i2,j2); 			
						//dot+=dotp*dotp;
						if(gradval>GRADTHRESHOLD&&((vecx*vecx+vecy*vecy)<DISTANCETHRESHOLD*DISTANCETHRESHOLD))
						{	
		...
		for(int i=src.cols/2;i<src.cols;i++)
		{
			if(srcInv.at<uchar>(j,i))
			{
				...
				{
					for(int i2=src.cols/2;i2<src.cols;i2++)
					{
						//Point p= contours[j2][i2];
						vecx=i2-i;
						vecy=j2-j;
