...
{
    line(img,Point(round(p.x-8*cos(float(bin*PI/9+PI/2))),round(p.y+8*sin(float(bin*PI/9+PI/2)))),Point(round(p.x+8*cos(float(bin*PI/9+PI/2))),round(p.y-8*sin(float(bin*PI/9+PI/2)))),Scalar(magnitude),1);    
}
...
Mat drawrhog(Mat image)
{
    ...
    Mat img= Mat::zeros(512,512,CV_8U);
    ...
    for(int i=0;i<225;i++)
    {
        ...
        for(int j=0;j<9;j++)
        {
            drawLine(img,Point(py,px),j,round(sqrt(storedHogFeatures[i][j])));
