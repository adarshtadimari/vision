...
{
    line(img,Point(round(p.x-8*cos(float(bin*PI/9))),round(p.y+8*sin(float(bin*PI/9)))),Point(round(p.x+8*cos(float(bin*PI/9))),round(p.y-8*sin(float(bin*PI/9)))),Scalar(magnitude),1);    
}
