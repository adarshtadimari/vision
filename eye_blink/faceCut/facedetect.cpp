                    ...
                    double scale, bool tryflip )
{
    ...
    for( vector<Rect>::const_iterator r = faces.begin(); r != faces.end(); r++, i++ )
    {
        ...
        if( 0.75 < aspect_ratio && aspect_ratio < 1.3 )
        {
            ...
            int heightf=radius*0.5;

            printf("size=%d\n",radius*2);
            Rect roi(center.x-widthf,center.y-heightf,2*widthf,2*heightf);
            int se=20;
            Rect roi_out(center.x-widthf-se,center.y-heightf-se,2*widthf+2*se,2*heightf+2*se);
            Rect roi_in(center.x-widthf+se,center.y-heightf+se,2*widthf-2*se,2*heightf-2*se);
