	...
	*/	
	...
	for(int j=0;j<src.rows;j++)
		for(int i=0;i<src.cols;i++)
		{
			...
			*/
			gradx=xDerivative.at<float>(j,i);
			grady=yDerivative.at<float>(j,i);
			gradval=gradx*gradx+grady*grady;
			if(gradmax<gradval)
				gradmax=gradval;
			if(gradval>GRADTHRESHOLD)
				grad.at<float>(j,i)=gradval;
	...
	Mat valuesSave;
	grad.convertTo(valuesSave,CV_8UC1,255);
	imshow("grad",grad);	
